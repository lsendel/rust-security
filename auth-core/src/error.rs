    ...
    fn into_response(self) -> Response {
        let (status, error_code, message) = match self {
            ...
            AuthError::Configuration(ref _msg) => {
                (StatusCode::INTERNAL_SERVER_ERROR, "server_error", "Server configuration error")
            }
            AuthError::Json(_) => {
                (StatusCode::BAD_REQUEST, "invalid_request", "Invalid JSON in request body")
            }
            AuthError::Io(_) => {
            ...
            #[cfg(feature = "jwt")]
            AuthError::Jwt(_) => {
                (StatusCode::INTERNAL_SERVER_ERROR, "server_error", "JWT processing error")
            }
            AuthError::Internal(ref _msg) => {
                (StatusCode::INTERNAL_SERVER_ERROR, "server_error", "Internal server error")
            }
        };
