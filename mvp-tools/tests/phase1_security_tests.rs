include!("regression_test_suite.rs");
...
impl RegressionTestSuite {
    ...
    pub async fn test_health_endpoints(&mut self) {
        ...
        let _policy_base_url = self.policy_base_url.clone();

        self.run_test("Health Endpoints", || {
            async move {
    ...
    pub async fn test_oauth_token_flow(&mut self) {
        ...
        self.run_test("OAuth2 Token Flow", |client, auth_base_url, _policy_base_url| async move {
            let response = client
    ...
    pub async fn test_token_revocation(&mut self) {
        ...
        self.run_test("Token Revocation", || async move {
            ...
            if revoke_response.status() != 200 {
                return Err(
                    format!("Token revocation failed: {}", revoke_response.status()).into(),
                );
            }
    ...
    pub async fn test_openid_connect(&mut self) {
        ...
        self.run_test("OpenID Connect", || async move {
            ...
            if oauth_metadata_response.status() != 200 {
                return Err(format!(
                    "OAuth metadata failed: {}",
                    oauth_metadata_response.status()
                )
                .into());
            }
    ...
    pub async fn test_jwks_endpoint(&mut self) {
        ...
        self.run_test("JWKS Endpoint", || async move {
            let jwks_response = client
                .get(&format!("{}/jwks.json", auth_base_url))
                .send()
                .await?;
    ...
    pub async fn test_scim_endpoints(&mut self) {
        ...
        self.run_test("SCIM Endpoints", || async move {
            ...
            let users_response = client
                .get(&format!("{}/scim/v2/Users", auth_base_url))
                .send()
                .await?;
            ...
            let groups_response = client
                .get(&format!("{}/scim/v2/Groups", auth_base_url))
                .send()
                .await?;
    ...
    pub async fn test_input_validation(&mut self) {
        ...
        self.run_test("Input Validation", || async move {
            ...
            let invalid_requests = vec![
                ...
                (
                    "grant_type=client_credentials&client_id=invalid&client_secret=invalid",
                    401,
                ),
            ];
            ...
            let all_correct = validation_results
                .iter()
