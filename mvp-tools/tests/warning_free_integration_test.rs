...
fn test_feature_combinations() {
    ...
    for config in test_configs {
        println!(
            ...
            config.component, config.features
        );

        let mut args = vec!["check", "-p", config.component, "--no-default-features"];
        let features_str = if !config.features.is_empty() {
            Some(config.features.join(","))
        } else {
            None
        };
        ...
        if config.should_compile {
            ...
            if config.expected_warnings == 0 {
                assert_eq!(
                    ...
                    config.component, config.features, warning_count
                );
            } else {
                // For feature-heavy components, just ensure they compile
                println!(
                    "    ✅ {} compiles with {} warnings",
                    config.component, warning_count
                );
            }
        }
    }
...
fn test_conditional_compilation_coverage() {
    ...
    let feature_gates = vec![
        (
            "rate-limiting",
            vec!["admin_replay_protection", "rate_limit_optimized"],
        ),
        ...
        (
            "threat-hunting",
            vec!["threat_intelligence", "threat_user_profiler"],
        ),
    ];
    ...
    for (feature, modules) in feature_gates {
        ...
        for module in modules {
            if stderr.contains(&format!("could not find `{}` in the crate root", module)) {
                println!(
                    "    ⚠️  Module {} properly gated behind feature {}",
                    module, feature
                );
            }
        }
