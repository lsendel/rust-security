...
impl RegressionTestSuite {
    ...
    pub async fn run_all_tests(&mut self) -> Result<TestSummary, Box<dyn std::error::Error>> {
        ...
        // TODO: Re-enable these test calls once method implementations are available
    ...
    where
        ...
        Fut: std::future::Future<
            Output = Result<Option<serde_json::Value>, Box<dyn std::error::Error + Send + Sync>>,
        >,
    {
        let start_time = std::time::Instant::now();
        print!("  {} ... ", test_name);

        match tokio::time::timeout(std::time::Duration::from_secs(30), test_fn()).await {
            Ok(Ok(details)) => {
    ...
    fn generate_summary(&self) -> TestSummary {
        ...
        let total_duration: std::time::Duration =
            self.test_results.values().map(|r| r.duration).sum();

        let failed_test_names: Vec<String> = self
            .test_results
            .values()
            .filter(|r| !r.passed)
            .map(|r| r.name.clone())
