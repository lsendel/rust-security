...
impl QuantumJwtManager {
    ...
    pub async fn initialize(&self) -> Result<()> {
        let key_pair = self.generate_quantum_keypair(self.config.default_security_level).await?;
        ...
        info!("Quantum JWT manager initialized with security level {}", self.config.default_security_level);
        Ok(())
    }
    ...
    async fn generate_post_quantum_keypair(&self, security_level: u8) -> Result<PostQuantumKeyPair> {
        let algorithm = match security_level {
        ...
        self.rng.fill(&mut private_key).map_err(|_| anyhow!("Failed to generate PQ private key"))?;
        self.rng.fill(&mut public_key).map_err(|_| anyhow!("Failed to generate PQ public key"))?;
    ...
    fn generate_ecdsa_private_key(&self, algorithm: &ClassicalAlgorithm) -> Result<Vec<u8>> {
        ...
        let mut private_key = vec![0u8; key_size];
        self.rng.fill(&mut private_key).map_err(|_| anyhow!("Failed to generate ECDSA private key"))?;
        
        Ok(private_key)
    }
    
    /// Derive ECDSA public key from private key
    fn derive_ecdsa_public_key(&self, private_key: &[u8], algorithm: &ClassicalAlgorithm) -> Result<Vec<u8>> {
    ...
    fn generate_key_id(&self) -> Result<String> {
        ...
        self.rng.fill(&mut bytes).map_err(|_| anyhow!("Failed to generate key ID"))?;
        Ok(hex::encode(bytes))
    }
    ...
    pub async fn create_token(&self, payload: QuantumJwtPayload) -> Result<String> {
        ...
        let signature = self.create_hybrid_signature(&signing_input, &key_pair).await?;
    ...
    async fn create_hybrid_signature(&self, data: &str, key_pair: &QuantumKeyPair) -> Result<HybridSignature> {
        // Classical signature
        let classical_sig = self.sign_classical(data.as_bytes(), &key_pair.classical).await?;
        
        // Post-quantum signature
        let pq_sig = self.sign_post_quantum(data.as_bytes(), &key_pair.post_quantum).await?;
        ...
        let combined_hash_b64 = base64::engine::general_purpose::STANDARD.encode(combined_hash.as_ref());
    ...
    async fn sign_post_quantum(&self, data: &[u8], key_pair: &PostQuantumKeyPair) -> Result<String> {
    ...
    pub async fn verify_token(&self, token: &str) -> Result<QuantumJwtPayload> {
        ...
        self.verify_hybrid_signature(&signing_input, &signature, &key_pair).await?;
    ...
    async fn verify_hybrid_signature(&self, data: &str, signature: &HybridSignature, key_pair: &QuantumKeyPair) -> Result<()> {
        // Verify classical signature
        self.verify_classical_signature(data.as_bytes(), &signature.classical, &key_pair.classical).await?;
        
        // Verify post-quantum signature
        self.verify_pq_signature(data.as_bytes(), &signature.post_quantum, &key_pair.post_quantum).await?;
        ...
        let expected_hash_b64 = base64::engine::general_purpose::STANDARD.encode(expected_hash.as_ref());
    ...
    /// Verify classical signature
    async fn verify_classical_signature(&self, data: &[u8], signature: &str, key_pair: &ClassicalKeyPair) -> Result<()> {
        ...
        let expected_signature_hash = digest::digest(&digest::SHA256, expected_signature_data.as_bytes());
        let expected_signature = base64::engine::general_purpose::STANDARD.encode(expected_signature_hash.as_ref());
    ...
    /// Verify post-quantum signature
    async fn verify_pq_signature(&self, data: &[u8], signature: &str, key_pair: &PostQuantumKeyPair) -> Result<()> {
    ...
    pub async fn rotate_keys(&self) -> Result<()> {
        let new_key_pair = self.generate_quantum_keypair(self.config.default_security_level).await?;
