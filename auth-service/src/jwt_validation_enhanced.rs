...
impl EnhancedJwtValidator {
        ...
        if let Some(ref analysis) = behavioral_analysis {
            if analysis.deviation_score > 0.7 {
                ...
                risk_factors.push(RiskFactor {
                    ...
                    contribution: 0.3,
                    description: "Behavioral pattern deviates significantly from baseline".to_string(),
                    severity: SecuritySeverity::High,
                });
            }
        }
    ...
    fn check_suspicious_characters(&self, claims: &EnhancedJwtClaims) -> Result<(), String> {
        ...
        let fields_to_check = [
            ...
            &claims.iss,
            &claims.aud,
            &claims.token_type.as_ref().unwrap_or(&String::new()),
            &claims.scope.as_ref().unwrap_or(&String::new()),
            &claims.nonce.as_ref().unwrap_or(&String::new()),
            &claims.client_id.as_ref().unwrap_or(&String::new()),
            &claims.session_id.as_ref().unwrap_or(&String::new()),
            &claims.ip_address.as_ref().unwrap_or(&String::new()),
            &claims.user_agent.as_ref().unwrap_or(&String::new()),
            &claims.device_fingerprint.as_ref().unwrap_or(&String::new()),
        ];
    ...
    /// Mock threat intelligence lookup
    fn mock_threat_intel_lookup(&self, ip: &str) -> ThreatIntelligence {
    ...
    fn calculate_risk_score(&self, violations: &[SecurityViolation], risk_factors: &[RiskFactor]) -> f64 {
    ...
    fn calculate_confidence_score(&self, _violations: &[SecurityViolation], _risk_factors: &[RiskFactor]) -> f64 {
        ...
        for violation in violations {
            match violation.violation_type {
                ...
                SecurityViolationType::IpBindingViolation => {
                    recommendations.push("Check IP address consistency and consider relaxing IP binding".to_string());
                }
                ...
                SecurityViolationType::HighRiskScore => {
                    recommendations.push("Review token risk assessment and consider additional validation".to_string());
                }
                SecurityViolationType::ThreatIntelligenceMatch => {
                    recommendations.push("Investigate IP reputation and consider blocking".to_string());
                }
                SecurityViolationType::BehavioralAnomaly => {
                    recommendations.push("Review behavioral patterns and update baselines".to_string());
                }
                _ => {
...
mod tests {
    ...
    fn test_suspicious_character_detection() {
        ...
        assert!(validator.check_suspicious_characters(&invalid_claims).is_err());
    }
