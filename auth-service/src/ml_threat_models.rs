...
impl MLThreatEngine {
    ...
    pub async fn analyze_threat(&self, features: &FeatureVector, user_id: Option<&str>) -> Result<MLThreatAssessment> {
        ...
        let confidence_interval = self.calculate_confidence_interval(
            overall_risk_score,
            vec![anomaly_score, behavioral_deviation, content_threat_score, temporal_anomaly_score],
        );

        // 8. Feature importance analysis
        let feature_importance = self.calculate_feature_importance(features, overall_risk_score).await;
    ...
    async fn analyze_user_behavior(&self, user_id: &str, features: &FeatureVector) -> Result<f64> {
        ...
        let user_model = models.entry(user_id.to_string()).or_insert_with(|| {
            UserBehaviorModel {
        ...
        if let Some(request_rate) = features.get("request_frequency") {
            let rate_deviation = user_model.request_rate_model.calculate_deviation(*request_rate);
            deviation_scores.push(rate_deviation);
        }

        // 2. Access pattern analysis
        if let Some(path_hash) = features.get("path_hash") {
            let pattern_deviation = user_model.access_pattern_model.calculate_deviation(*path_hash as u32);
            deviation_scores.push(pattern_deviation);
        }

        // 3. Content similarity analysis
        let content_features: Vec<f64> = features.iter()
            .filter(|(k, _)| k.starts_with("content_"))
            .map(|(_, v)| *v)
            .collect();
        
        if !content_features.is_empty() {
            let similarity_deviation = user_model.content_similarity_model.calculate_deviation(&content_features);
    ...
    async fn retrain_user_model(&self, user_model: &mut UserBehaviorModel) -> Result<()> {
        ...
        let request_rates: Vec<f64> = training_data.iter()
        ...
        let path_hashes: Vec<u32> = training_data.iter()
        ...
        let content_features: Vec<Vec<f64>> = training_data.iter()
            .map(|f| {
                f.iter()
                    .filter(|(k, _)| k.starts_with("content_"))
                    .map(|(_, v)| *v)
        ...
        let temporal_data: Vec<(f64, DateTime<Utc>)> = training_data.iter()
            .enumerate()
            .filter_map(|(i, f)| {
    ...
    fn calculate_ensemble_risk_score(
        ...
        let categorical_score = threat_categories.iter()
            .map(|t| t.probability * match t.severity {
    ...
    fn calculate_confidence_interval(&self, risk_score: f64, model_scores: Vec<f64>) -> (f64, f64) {
        ...
        let score_variance = {
            ...
            let variance: f64 = model_scores.iter()
                ...
                .sum::<f64>() / model_scores.len() as f64;
            variance.sqrt()
        };
    ...
    async fn calculate_feature_importance(&self, features: &FeatureVector, baseline_score: f64) -> HashMap<String, f64> {
    ...
    async fn cleanup_old_user_models(&self) -> Result<()> {
        ...
        if before_count > after_count {
            info!("Cleaned up {} inactive user models", before_count - after_count);
        }
...
impl MarkovChain {
    ...
    pub fn fit(&mut self, sequence: &[u32]) {
        ...
        for window in sequence.windows(2) {
            ...
            *self.transition_matrix.entry(from).or_default().entry(to).or_insert(0.0) += 1.0;
        }
    ...
    pub fn calculate_deviation(&self, current_state: u32) -> f64 {
        ...
        if total_states == 0 { return 0.5; }

        let state_probability = self.state_counts.get(&current_state)
...
impl CosineSimilarityModel {
    ...
    pub fn calculate_deviation(&self, vector: &[f64]) -> f64 {
        ...
        let max_similarity = self.reference_vectors.iter()
...
impl AnomalyDetector {
    ...
    pub fn quick_anomaly_check(&self, features: &FeatureVector) -> f64 {
        ...
        let suspicious_features = features.iter()
...
impl ThreatClassifierEnsemble {
    ...
    pub async fn classify_threats(&self, features: &FeatureVector, anomaly_score: f64) -> Result<Vec<ThreatCategory>> {
...
impl ContentAnalysisModel {
    ...
    pub async fn analyze_content_threats(&self, features: &FeatureVector) -> Result<f64> {
        ...
        if let Some(&entropy) = features.get("content_entropy") {
            if entropy > 4.5 { threat_indicators += 1.0; }
...
impl ModelPerformanceTracker {
    ...
    pub fn record_prediction(&mut self, risk_score: f64, processing_time_ms: u64) {
        self.prediction_history.push_back((risk_score, processing_time_ms));
    ...
    pub fn detect_performance_drift(&self) -> bool {
        ...
        let historical_accuracy = self.daily_accuracy.iter().sum::<f64>() / self.daily_accuracy.len() as f64;
...
mod tests {
    ...
    async fn test_ml_threat_engine_initialization() {
        ...
        let assessment = engine.analyze_threat(&features, Some("test_user")).await.unwrap();
