...
impl EnhancedRateLimiter {
        ...
        let task = tokio::spawn(async move {
            ...
            while is_running.load(Ordering::Relaxed) {
                ...
                if entries_map.len() > config.adaptive_cleanup_threshold {
                    ...
                    warn!("Adaptive cleanup removed {} entries to prevent memory exhaustion", excess);
                    metrics.rate_limited_requests.fetch_add(excess as u64, Ordering::Relaxed);
                }
            }
        });
    ...
    async fn start_ml_training_task(&self) {
        ...
        tokio::spawn(async move {
            let mut interval = tokio::time::interval(Duration::from_secs((config.ml_model_update_frequency_minutes * 60) as u64));
            ...
            while is_running.load(Ordering::Relaxed) {
                ...
                let mut model = ml_model.as_ref().clone();
    ...
    async fn start_behavioral_analysis_task(&self) {
        ...
        tokio::spawn(async move {
            ...
            while is_running.load(Ordering::Relaxed) {
                ...
                let retention_cutoff = now.saturating_sub((config.behavioral_baselines_retention_days as u64) * 86400);
                
                let mut baselines = behavioral_baselines.write().await;
                baselines.retain(|_, baseline| baseline.updated > retention_cutoff);
                
                debug!("Behavioral analysis cleanup - retained {} baselines", baselines.len());
            }
        });
    }
    ...
    pub async fn check_rate_limit(
        ...
        if entry.tokens > 0 {
            ...
            if entry.trust_score < 1.0 {
                entry.trust_score = (entry.trust_score + self.config.behavioral_trust_decay_rate).min(1.0);
            }
    ...
    fn refill_tokens(&self, entry: &mut EnhancedRateLimitEntry, endpoint: &str) {
        ...
        if elapsed >= self.config.window_duration_secs {
            ...
            let refill_amount = (elapsed as f64 / self.config.window_duration_secs as f64) * base_limit * entry.adaptive_multiplier;
    ...
    fn prune_request_history(&self, entry: &mut EnhancedRateLimitEntry) {
        ...
        entry.request_history.retain(|&timestamp| {
    ...
    async fn calculate_effective_limit(
        ...
        if self.config.enable_context_aware_limits {
            let context_score = self.calculate_context_score(client_ip, client_id, endpoint, user_agent).await;
            effective_limit *= context_score;
        }
        
        // Apply machine learning predictions if enabled
        if self.config.enable_ml_prediction {
            let features = self.extract_ml_features(entry, client_ip, client_id, endpoint).await;
    ...
    fn detect_behavioral_deviation(
        ...
        let recent_requests: Vec<u64> = entry.request_history
            .iter()
    ...
    async fn build_behavioral_baseline(
        ...
        self.metrics.behavioral_analyses.fetch_add(1, Ordering::Relaxed);
    }
    ...
    async fn calculate_context_score(
        ...
        self.metrics.context_based_decisions.fetch_add(1, Ordering::Relaxed);
    ...
    async fn extract_ml_features(
        ...
        features.insert("recent_requests".to_string(), entry.request_history.len() as f64);
    ...
    fn apply_progressive_penalty(&self, entry: &mut EnhancedRateLimitEntry) {
        if entry.violation_count > 1 {
            let penalty_factor = self.config.adaptive_backoff_factor.powi(entry.violation_count as i32);
            entry.adaptive_multiplier /= penalty_factor;
            entry.adaptive_multiplier = entry.adaptive_multiplier.max(self.config.adaptive_min_multiplier);
        }
    }
    ...
    fn calculate_retry_after(&self, entry: &EnhancedRateLimitEntry, effective_limit: u32) -> u64 {
        ...
        if self.config.progressive_delays_enabled && entry.violation_count > 1 {
            let delay_multiplier = self.config.adaptive_backoff_factor.powi(entry.violation_count as i32);
    ...
    fn get_burst_limit(&self, endpoint: &str) -> u32 {
        match endpoint {
            ...
            "/admin/" if endpoint.starts_with("/admin/") => self.config.admin_requests_per_minute * 2,
    ...
    pub async fn get_adaptive_stats(&self) -> AdaptiveStats {
        ...
        AdaptiveStats {
            ...
            min_adaptive_multiplier: multipliers.iter().fold(f64::INFINITY, |a, &b| a.min(b)).min(1.0),
            max_adaptive_multiplier: multipliers.iter().fold(0.0, |a, &b| a.max(b)).max(1.0),
        }
    }
}
...
pub async fn enhanced_rate_limit_middleware(
    ...
    let key = format!(
        "rl:{}:{}:{}",
        client_ip.as_ref().map_or("unknown", |ip| &ip.to_string()),
    ...
    match result {
        ...
        EnhancedRateLimitResult::RateLimited { retry_after, limit, remaining, reset_time } => {
        ...
        EnhancedRateLimitResult::Blocked { reason, duration_minutes } => {
            ...
            if duration_minutes > 0 {
                if let Some(headers) = response.headers_mut() {
                    headers.insert("Retry-After", (duration_minutes * 60).to_string().parse().unwrap());
                }
            }
...
pub fn create_default_enhanced_rate_limiter() -> EnhancedRateLimiter {
    ...
    if let Ok(enable_adaptive) = std::env::var("ENABLE_ADAPTIVE_LIMITS") {
        config.enable_adaptive_limits = enable_adaptive == "1" || enable_adaptive.to_lowercase() == "true";
    }
    ...
    if let Ok(enable_behavioral) = std::env::var("ENABLE_BEHAVIORAL_ANALYSIS") {
        config.enable_behavioral_analysis = enable_behavioral == "1" || enable_behavioral.to_lowercase() == "true";
    }
...
mod tests {
    ...
    async fn test_behavioral_baseline_building() {
        ...
        limiter.build_behavioral_baseline(&mut entry, Some("test-client")).await;
    ...
    fn test_context_score_calculation() {
        ...
        // Test normal context
        let score = limiter.analyze_user_agent("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36");
        assert_eq!(score, 1.0);
    ...
    async fn test_metrics_tracking() {
        ...
        assert_eq!(limiter.get_metrics().total_requests.load(Ordering::Relaxed), 0);
        assert_eq!(limiter.get_metrics().allowed_requests.load(Ordering::Relaxed), 0);
        assert_eq!(limiter.get_metrics().rate_limited_requests.load(Ordering::Relaxed), 0);
        assert_eq!(limiter.get_metrics().blocked_requests.load(Ordering::Relaxed), 0);
