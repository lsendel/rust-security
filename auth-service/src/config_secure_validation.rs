...
impl SecureConfig {
    ...
    pub fn from_env() -> Result<Self, ConfigError> {
        ...
        let weak_secrets = ["secret", "password", "default", "fallback-secret-key", "dev-secret"];
        ...
        let app_env = match std::env::var("APP_ENV").as_deref() {
            ...
            Ok(other) => return Err(ConfigError::Invalid {
                var: "APP_ENV",
        ...
        if let Some(ref url) = database_url {
            if url.contains("password=") && !url.starts_with("postgresql://") && !url.starts_with("postgres://") {
                tracing::warn!(
                    target: "security_config",
...
mod tests {
    ...
    fn test_production_wildcard_cors_rejection() {
        std::env::set_var("JWT_SECRET", "very-secure-secret-key-at-least-32-characters-long");
