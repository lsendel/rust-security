...
impl AlertCorrelationEngine {
    ...
    pub async fn process_alert(
        ...
        if let Some(statistical_result) = self.statistical_correlator.find_correlations(alert).await? {
        ...
        let processing_time = start_time.elapsed().as_millis() as f64;
        {
            ...
            metrics.avg_processing_time_ms = 
                (metrics.avg_processing_time_ms * (metrics.total_alerts_processed - 1) as f64 + processing_time) 
    ...
    async fn add_to_cache(
        ...
        let mut cache = self.alert_cache.entry(window_key.clone()).or_insert_with(|| {
            TimeWindowCache {
    ...
    async fn evaluate_correlation_rule(
        ...
        if related_alerts.len() + 1 >= rule.min_events as usize {
            ...
            let correlation_result = CorrelationResult {
                id: Uuid::new_v4().to_string(),
                alerts: [vec![alert.id.clone()], related_alerts.iter().map(|a| a.id.clone()).collect()].concat(),
                rule_id: rule.id.clone(),
                score: correlation_score,
                timestamp: Utc::now(),
                metadata: [
                    ("rule_name".to_string(), serde_json::Value::String(rule.name.clone())),
                    ("alert_count".to_string(), serde_json::Value::Number((related_alerts.len() + 1).into())),
                    ("time_window_minutes".to_string(), serde_json::Value::Number(rule.time_window_minutes.into())),
                ].into_iter().collect(),
            };
    ...
    fn matches_correlation_conditions(&self, rule: &CorrelationRule, alert: &SecurityAlert) -> bool {
        for condition in &rule.conditions {
            match condition.field.as_str() {
                "alert_type" => {
                    let alert_type_str = format!("{:?}", alert.alert_type);
                    if !self.evaluate_condition_value(&condition.value, &alert_type_str, &condition.operator) {
                        return false;
                    }
                }
                "severity" => {
                    let severity_str = format!("{:?}", alert.severity);
                    if !self.evaluate_condition_value(&condition.value, &severity_str, &condition.operator) {
                        return false;
                    }
                }
                "source_ip" => {
                    if let Some(ref source_ip) = alert.source_ip {
                        if !self.evaluate_condition_value(&condition.value, source_ip, &condition.operator) {
                            return false;
                        }
                ...
                "user_id" => {
                    if let Some(ref user_id) = alert.user_id {
                        if !self.evaluate_condition_value(&condition.value, user_id, &condition.operator) {
                            return false;
                        }
                    } else if condition.required {
                        return false;
                ...
                _ => {
                    ...
                    if let Some(metadata_value) = alert.metadata.get(&condition.field) {
                        if let Some(metadata_str) = metadata_value.as_str() {
                            if !self.evaluate_condition_value(&condition.value, metadata_str, &condition.operator) {
                                return false;
                            }
                        }
                    } else if condition.required {
    ...
    /// Load default correlation rules
    async fn load_default_correlation_rules(&self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        rules.push(CorrelationRule {
            ...
            action: CorrelationAction {
                ...
                parameters: [
                    ("title".to_string(), serde_json::Value::String("Multiple Authentication Failures".to_string())),
    ...
    /// Load default attack patterns
    async fn load_default_attack_patterns(&self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    async fn start_correlation_processor(&self) {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                debug!("Correlation metrics: {} alerts processed, {} correlations found", 
                    metrics_guard.total_alerts_processed, 
                    metrics_guard.total_correlations
                );
            }
        });
    }
    ...
    async fn start_cleanup_processor(&self) {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                let cutoff_time = Utc::now() - Duration::minutes(config_guard.correlation_window_minutes as i64 * 2);
                drop(config_guard);
