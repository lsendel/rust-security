...
mod security_tests {
    ...
    fn test_token_binding_uses_secure_salt() {
        ...
        assert_ne!(binding1, binding2, "Token bindings should not be identical due to timestamp");
    ...
    fn test_pkce_code_verifier_security() {
        ...
        assert!(verifier1.chars().all(|c| c.is_ascii_alphanumeric() || c == '-' || c == '_'));
    }
    ...
    async fn test_rate_limiting_security() {
        ...
        assert!(matches!(result, Err(RateLimitError::IpLimitExceeded { .. })));
    }
    ...
    fn test_timing_attack_prevention() {
        ...
        assert!(time_diff.as_millis() < 1, "Timing difference too large: {:?}", time_diff);
    }
    ...
    fn test_cors_security() {
        ...
        std::env::set_var("ALLOWED_ORIGINS", "https://example.com,https://app.example.com");
