...
impl KeyManagementService {
    ...
    pub async fn get_jwks(&self) -> Value {
        let keys = self.keys.read().await;
        let jwk_keys: Vec<Value> = keys.values()
    ...
    async fn generate_rsa_key(&self, kid: &str) -> Result<(EncodingKey, DecodingKey, Value), AuthError> {
...
mod tests {
    ...
    async fn test_metrics_collection() {
        ...
        kms.initialize().await.unwrap();
        kms.rotate_keys("test").await.unwrap();
        kms.revoke_key(&kms.active_key_id.read().await.clone().unwrap(), "test", "test").await.unwrap();
