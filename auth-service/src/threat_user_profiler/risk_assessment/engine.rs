...
impl RiskAssessmentEngine {
    ...
    pub async fn assess_risk(
        ...
        let temporal_risks = self.assess_temporal_risks(&profile.feature_vector.temporal_features).await?;
        risk_factors.extend(temporal_risks);

        // Location risk factors
        let location_risks = self.assess_location_risks(&profile.feature_vector.location_features).await?;
        risk_factors.extend(location_risks);

        // Device risk factors
        let device_risks = self.assess_device_risks(&profile.feature_vector.device_features).await?;
        risk_factors.extend(device_risks);

        // Network risk factors
        let network_risks = self.assess_network_risks(&profile.feature_vector.network_features).await?;
        risk_factors.extend(network_risks);

        // Activity risk factors
        let activity_risks = self.assess_activity_risks(&profile.feature_vector.activity_features).await?;
        risk_factors.extend(activity_risks);

        // Peer comparison risks
        let peer_risks = self.peer_analyzer.assess_peer_risks(profile, peer_profiles).await?;
        ...
        let overall_risk_score = self.scoring_algorithm
            .calculate_overall_risk(&risk_factors, &config.risk_aggregation_method).await?;
        ...
        debug!("Risk assessment completed for user {}: score={:.3}, category={:?}, confidence={:.3}",
    ...
    async fn assess_temporal_risks(
        ...
        if temporal_features.login_frequency > 50.0 { // Unusually high frequency
            risk_factors.push(RiskFactor {
                factor_name: "high_login_frequency".to_string(),
                risk_score: (temporal_features.login_frequency / 50.0).min(1.0),
                weight: config.base_risk_weights.get("temporal_anomaly").unwrap_or(&0.3) * 0.3,
                description: "Unusually high login frequency detected".to_string(),
                evidence: vec![format!("Login frequency: {:.1} per day", temporal_features.login_frequency)],
            });
        }
        ...
        if temporal_features.session_duration_std > temporal_features.session_duration_avg * 2.0 {
            risk_factors.push(RiskFactor {
                ...
                risk_score: (temporal_features.session_duration_std / temporal_features.session_duration_avg).min(1.0),
                weight: config.base_risk_weights.get("temporal_anomaly").unwrap_or(&0.3) * 0.2,
                description: "Highly irregular session duration patterns".to_string(),
                evidence: vec![
                    format!("Average session: {:.1} minutes", temporal_features.session_duration_avg),
                    format!("Standard deviation: {:.1} minutes", temporal_features.session_duration_std),
                ],
            });
        }

        // Off-hours activity
        let off_hours_activity = self.calculate_off_hours_activity(&temporal_features.active_hours_pattern);
        if off_hours_activity > 0.3 {
            risk_factors.push(RiskFactor {
                factor_name: "off_hours_activity".to_string(),
                risk_score: off_hours_activity,
                weight: config.base_risk_weights.get("temporal_anomaly").unwrap_or(&0.3) * 0.25,
                description: "Significant activity during off-hours".to_string(),
                evidence: vec![format!("Off-hours activity: {:.1}%", off_hours_activity * 100.0)],
            });
        }

        // Weekend activity anomalies
        let weekend_activity = self.calculate_weekend_activity(&temporal_features.day_of_week_pattern);
        if weekend_activity > 0.4 {
            risk_factors.push(RiskFactor {
                ...
                evidence: vec![format!("Weekend activity: {:.1}%", weekend_activity * 100.0)],
            });
        }
    ...
    async fn assess_location_risks(
        ...
        if location_features.location_entropy > 3.0 {
            risk_factors.push(RiskFactor {
                ...
                weight: config.base_risk_weights.get("location_anomaly").unwrap_or(&0.25) * 0.4,
                ...
                evidence: vec![
                    ...
                    format!("Location entropy: {:.2}", location_features.location_entropy),
                ],
            });
        }

        // High travel velocity (impossible travel)
        if location_features.travel_velocity > 1000.0 { // km/h
            risk_factors.push(RiskFactor {
                factor_name: "impossible_travel".to_string(),
                risk_score: (location_features.travel_velocity / 2000.0).min(1.0),
                weight: config.base_risk_weights.get("location_anomaly").unwrap_or(&0.25) * 0.5,
                description: "Impossible travel velocity detected".to_string(),
                evidence: vec![format!("Travel velocity: {:.1} km/h", location_features.travel_velocity)],
            });
        }
        ...
        if location_features.location_consistency < 0.3 {
            risk_factors.push(RiskFactor {
                ...
                weight: config.base_risk_weights.get("location_anomaly").unwrap_or(&0.25) * 0.3,
                description: "Inconsistent location patterns".to_string(),
                evidence: vec![format!("Location consistency: {:.2}", location_features.location_consistency)],
            });
        }
        ...
        if location_features.geofence_violations > 0 {
            risk_factors.push(RiskFactor {
                ...
                weight: config.base_risk_weights.get("location_anomaly").unwrap_or(&0.25) * 0.6,
                description: "Geofence policy violations detected".to_string(),
                evidence: vec![format!("Violations: {}", location_features.geofence_violations)],
            });
        }
    ...
    async fn assess_device_risks(
        ...
        if device_features.unique_devices > 10 {
            risk_factors.push(RiskFactor {
                ...
                weight: config.base_risk_weights.get("device_anomaly").unwrap_or(&0.2) * 0.4,
                description: "User accessing from excessive number of devices".to_string(),
                evidence: vec![format!("Unique devices: {}", device_features.unique_devices)],
            });
        }
        ...
        if device_features.device_consistency < 0.5 {
            risk_factors.push(RiskFactor {
                ...
                weight: config.base_risk_weights.get("device_anomaly").unwrap_or(&0.2) * 0.3,
                description: "Inconsistent device usage patterns".to_string(),
                evidence: vec![format!("Device consistency: {:.2}", device_features.device_consistency)],
            });
        }
        ...
        if device_features.new_device_frequency > 0.5 {
            risk_factors.push(RiskFactor {
                ...
                weight: config.base_risk_weights.get("device_anomaly").unwrap_or(&0.2) * 0.5,
                description: "Frequently using new devices".to_string(),
                evidence: vec![format!("New device frequency: {:.2}", device_features.new_device_frequency)],
            });
        }
    ...
    async fn assess_network_risks(
        ...
        if network_features.suspicious_ip_interactions > 0 {
            risk_factors.push(RiskFactor {
                ...
                weight: config.base_risk_weights.get("network_anomaly").unwrap_or(&0.15) * 0.6,
                description: "Interactions with suspicious IP addresses".to_string(),
                evidence: vec![format!("Suspicious IPs: {}", network_features.suspicious_ip_interactions)],
            });
        }
        ...
        if network_features.tor_usage_frequency > 0.0 {
            risk_factors.push(RiskFactor {
                ...
                weight: config.base_risk_weights.get("network_anomaly").unwrap_or(&0.15) * 0.8,
                description: "Tor network usage detected".to_string(),
                evidence: vec![format!("Tor usage frequency: {:.2}", network_features.tor_usage_frequency)],
            });
        }
        ...
        if network_features.ip_geolocation_consistency < 0.7 {
            risk_factors.push(RiskFactor {
                ...
                weight: config.base_risk_weights.get("network_anomaly").unwrap_or(&0.15) * 0.4,
                description: "Inconsistent IP geolocation patterns".to_string(),
                evidence: vec![format!("IP consistency: {:.2}", network_features.ip_geolocation_consistency)],
            });
        }
    ...
    async fn assess_activity_risks(
        ...
        if activity_features.failed_action_rate > 0.1 {
            risk_factors.push(RiskFactor {
                ...
                weight: config.base_risk_weights.get("activity_anomaly").unwrap_or(&0.1) * 0.7,
                description: "High rate of failed actions".to_string(),
                evidence: vec![format!("Failed action rate: {:.1}%", activity_features.failed_action_rate * 100.0)],
            });
        }
        ...
        if activity_features.action_diversity < 0.3 {
            risk_factors.push(RiskFactor {
                ...
                weight: config.base_risk_weights.get("activity_anomaly").unwrap_or(&0.1) * 0.5,
                description: "Low diversity in user actions (potential automation)".to_string(),
                evidence: vec![format!("Action diversity: {:.2}", activity_features.action_diversity)],
            });
        }
    ...
    fn calculate_off_hours_activity(&self, hourly_pattern: &[f64]) -> f64 {
        ...
        let off_hours_activity: f64 = off_hours_indices.iter()
            .map(|&i| hourly_pattern[i])
            .sum();
    ...
    pub async fn get_risk_trend(&self, user_id: Uuid) -> Option<Vec<f64>> {
        ...
        history.get(&user_id).map(|assessments| {
            assessments.iter().map(|a| a.overall_risk_score).collect()
        })
    }
}
...
mod tests {
    ...
    async fn test_risk_assessment() {
        ...
        let assessment = engine.assess_risk(profile.user_id, &profile, &[]).await.unwrap();
