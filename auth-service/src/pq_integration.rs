...
use crate::post_quantum_crypto::{
    get_pq_manager, initialize_post_quantum_crypto, PQAlgorithm, SecurityLevel, MigrationMode
};
use crate::pq_jwt::{get_pq_jwt_manager, create_pq_access_token};
use crate::pq_key_management::{
    get_pq_key_manager, initialize_pq_key_management, KeyOperation, EmergencyTrigger
};
use crate::pq_migration::{
    get_migration_manager, initialize_migration_management, run_benchmark, 
    generate_compliance_report, PerformanceBenchmark
};
use crate::security_logging::{SecurityLogger, SecurityEvent, SecurityEventType, SecuritySeverity};
use crate::{AuthError, AppState};
...
pub async fn create_pq_jwt(
    ...
    let token = create_enhanced_access_token(
        ...
        expires_in,
        force_pq,
    ).await.map_err(|e| AuthError::InternalError(e))?;
...
pub async fn run_pq_benchmark(
    ...
    let benchmark = run_benchmark(algorithm, iterations).await
...
pub async fn force_key_rotation(
    ...
    let rotated_keys = key_manager.rotate_keys(reason).await
        .map_err(|e| AuthError::InternalError(e))?;
...
pub async fn start_migration_phase(
    ...
    migration_manager.start_phase(&request.phase_id).await
        .map_err(|e| AuthError::InternalError(e))?;
...
pub async fn emergency_rollback(
    ...
    if !request.confirm {
        return Err(AuthError::InvalidRequest("Emergency rollback requires confirmation".to_string()));
    }
    ...
    let key_manager = get_pq_key_manager();
    let rotated_keys = key_manager.emergency_rotation(trigger).await
        .map_err(|e| AuthError::InternalError(e))?;
...
pub async fn pq_health_check() -> Result<Json<serde_json::Value>, AuthError> {
    ...
    if total_operations > 0 {
        for (alg, perf) in &key_stats.performance_summary {
            if perf.error_rate > 5.0 { // 5% error rate threshold
                health["status"] = "degraded".into();
                health["issues"] = vec![format!("High error rate for {}: {:.2}%", alg, perf.error_rate)].into();
            }
        }
    }
...
pub async fn migrate_token_issuance(
    ...
    if let Ok(token) = create_enhanced_access_token(
        ...
        state.token_store.set_refresh(&refresh_token, crate::REFRESH_TOKEN_EXPIRY_SECONDS).await?;
    ...
    crate::issue_new_token(state, scope, client_id, make_id_token, subject).await
