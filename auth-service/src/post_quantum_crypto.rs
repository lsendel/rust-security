...
impl PQCryptoManager {
        ...
        let algorithm = algorithm.unwrap_or_else(|| {
            if self.config.enable_hybrid {
                PQAlgorithm::Hybrid {
                    ...
                    post_quantum: Box::new(PQAlgorithm::Dilithium(self.config.default_security_level)),
                }
            } else {
    ...
    #[cfg(feature = "hybrid-crypto")]
    async fn generate_classical_key_data(&self, algorithm: &ClassicalAlgorithm) -> Result<ClassicalKeyData> {
        match algorithm {
    ...
    #[cfg(not(feature = "hybrid-crypto"))]
    async fn generate_classical_key_data(&self, _algorithm: &ClassicalAlgorithm) -> Result<ClassicalKeyData> {
        Ok(ClassicalKeyData::Placeholder)
    }

    /// Generate JWK representation for the key
    async fn generate_jwk(&self, algorithm: &PQAlgorithm, key_data: &PQKeyData) -> Result<Option<serde_json::Value>> {
        match (algorithm, key_data) {
            ...
            #[cfg(feature = "hybrid-crypto")]
            (PQAlgorithm::Hybrid { classical, post_quantum }, PQKeyData::Hybrid { classical: classical_data, post_quantum: pq_data }) => {
    ...
    #[cfg(feature = "hybrid-crypto")]
    async fn generate_classical_jwk(&self, algorithm: &ClassicalAlgorithm, key_data: &ClassicalKeyData) -> Result<serde_json::Value> {
        match (algorithm, key_data) {
    ...
    #[cfg(not(feature = "hybrid-crypto"))]
    async fn generate_classical_jwk(&self, _algorithm: &ClassicalAlgorithm, _key_data: &ClassicalKeyData) -> Result<serde_json::Value> {
        Ok(serde_json::json!({}))
    }
    ...
    async fn sign_with_key_material(&self, data: &[u8], key_material: &PQKeyMaterial) -> Result<Vec<u8>> {
        match &key_material.key_data {
            #[cfg(feature = "post-quantum")]
            PQKeyData::Dilithium { private_key, .. } => {
                match key_material.security_level {
                    SecurityLevel::Level1 => {
    ...
    #[cfg(feature = "hybrid-crypto")]
    async fn sign_classical(&self, data: &[u8], classical_data: &ClassicalKeyData) -> Result<Vec<u8>> {
        match classical_data {
            ClassicalKeyData::Ed25519 { private_key, .. } => {
                use ed25519_dalek::Signer;
                let signing_key = Ed25519SigningKey::from_bytes(
                    private_key.as_slice().try_into()
    ...
    #[cfg(not(feature = "hybrid-crypto"))]
    async fn sign_classical(&self, _data: &[u8], _classical_data: &ClassicalKeyData) -> Result<Vec<u8>> {
    ...
    pub async fn verify(&self, data: &[u8], signature: &[u8], kid: &str) -> Result<bool> {
        let keys = PQ_KEYS.read().await;
        let key_material = keys.get(kid)
            .ok_or_else(|| anyhow!("Key not found: {}", kid))?;

        self.verify_with_key_material(data, signature, key_material).await
    }

    async fn verify_with_key_material(&self, data: &[u8], signature: &[u8], key_material: &PQKeyMaterial) -> Result<bool> {
        match &key_material.key_data {
            #[cfg(feature = "post-quantum")]
            PQKeyData::Dilithium { public_key, .. } => {
                match key_material.security_level {
                    SecurityLevel::Level1 => {
            ...
            PQKeyData::Hybrid { classical, post_quantum } => {
                ...
                let classical_sig_len = u32::from_be_bytes([
                ...
                let pq_valid = self.verify_with_key_material(data, pq_sig, &pq_key_material).await?;
    ...
    #[cfg(feature = "hybrid-crypto")]
    async fn verify_classical(&self, data: &[u8], signature: &[u8], classical_data: &ClassicalKeyData) -> Result<bool> {
        match classical_data {
            ClassicalKeyData::Ed25519 { public_key, .. } => {
                use ed25519_dalek::{Verifier, Signature};
                let verifying_key = Ed25519VerifyingKey::from_bytes(
                    public_key.as_slice().try_into()
    ...
    #[cfg(not(feature = "hybrid-crypto"))]
    async fn verify_classical(&self, _data: &[u8], _signature: &[u8], _classical_data: &ClassicalKeyData) -> Result<bool> {
        Err(anyhow!("Hybrid cryptography features not enabled"))
    }
    ...
    pub async fn current_signing_key_id(&self) -> Option<String> {
        ...
        keys.values()
            .filter(|k| {
                k.expires_at.map_or(true, |exp| current_timestamp() < exp)
            })
            .max_by_key(|k| k.created_at)
            .map(|k| k.kid.clone())
    }
    ...
    pub async fn jwks_document(&self) -> serde_json::Value {
        let keys = PQ_KEYS.read().await;
        let jwk_keys: Vec<serde_json::Value> = keys.values()
            .filter_map(|k| k.jwk.clone())
            .collect();
    ...
    pub async fn rotate_expired_keys(&self) -> Result<Vec<String>> {
        ...
        let keys_to_rotate: Vec<String> = {
            ...
            keys.values()
                .filter(|k| {
                    k.expires_at.map_or(false, |exp| current_time >= exp)
                })
                .map(|k| k.kid.clone())
                .collect()
        };
    ...
    pub async fn get_performance_metrics(&self) -> PQPerformanceMetrics {
        ...
        let algorithm_distribution: HashMap<String, usize> = keys.values()
        ...
        PQPerformanceMetrics {
            ...
            next_rotation: keys.values()
                .filter_map(|k| k.expires_at)
                .min(),
        }
    }
...
static PQ_MANAGER: Lazy<PQCryptoManager> = Lazy::new(|| {
    PQCryptoManager::default()
});
