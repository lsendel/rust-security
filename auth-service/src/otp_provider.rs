...
pub struct TwilioSender { pub account_sid: String, pub auth_token: String, pub from: String }
#[async_trait]
impl OtpSender for TwilioSender {
    ...
    async fn send_email(&self, _to: &str, _subject: &str, _body: &str) -> anyhow::Result<()> { Ok(()) }
}

pub struct SesSender { /* add AWS SDK client here */ }
#[async_trait]
impl OtpSender for SesSender {
    async fn send_sms(&self, _to: &str, _msg: &str) -> anyhow::Result<()> { Ok(()) }
    async fn send_email(&self, _to: &str, _subject: &str, _body: &str) -> anyhow::Result<()> { Ok(()) }
}
