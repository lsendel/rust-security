...
impl AdvancedCircuitBreaker {
    ...
    async fn check_call_allowed(&self) -> Result<(), CircuitBreakerError> {
        ...
        match metrics.state {
            ...
            CircuitState::Open => {
                ...
                let now = Instant::now();
                if now.saturating_duration_since(metrics.state_transition_time)
    ...
    async fn update_adaptive_config(&self) {
        ...
        if failure_rate > 0.3 {
            ...
            adaptive_config.failure_threshold =
                adaptive_config.failure_threshold.saturating_sub(1).max(2);
            adaptive_config.failure_rate_threshold =
                (adaptive_config.failure_rate_threshold * 0.9).max(0.2);
            ...
            adaptive_config.failure_rate_threshold =
                (adaptive_config.failure_rate_threshold * 1.1).min(0.8);
        }
...
impl CircuitBreakerRegistry {
    ...
    pub async fn get_system_health(&self) -> SystemHealthStatus {
        ...
        let healthy = statuses
            .iter()
            .filter(|s| s.health_score > 0.8 && s.state == CircuitState::Closed)
            .count();
        let degraded = statuses
            .iter()
            .filter(|s| s.health_score > 0.5 && s.health_score <= 0.8)
            .count();
        let failed = statuses
            .iter()
            .filter(|s| s.health_score <= 0.5 || s.state == CircuitState::Open)
            .count();

        let overall_health =
            statuses.iter().map(|s| s.health_score).sum::<f64>() / statuses.len() as f64;
