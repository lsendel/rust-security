...
async fn register_sets_cookies_when_keys_available() {
    ...
    auth_service::infrastructure::crypto::keys::initialize_keys()
        .await
        .expect("init keys");
    ...
    let req = RegisterRequest {
        ...
        password: "StrongP@ssw0rd1".to_string(),
        name: "User".to_string(),
    };
    ...
    assert!(cookies
        .iter()
        .any(|h| h.to_str().unwrap().starts_with("access_token=")));
    assert!(cookies
        .iter()
        .any(|h| h.to_str().unwrap().starts_with("csrf_token=")));
