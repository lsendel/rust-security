...
use auth_service::{
    domain::{
        entities::{User, Session, Token, TokenType},
        value_objects::{UserId, Email, PasswordHash},
    },
...
async fn test_user_registration_flow() {
    ...
    let is_valid = crypto_service.verify_password(&password, &password_hash).await.unwrap();
    assert!(is_valid);
    
    // Test wrong password
    let is_invalid = crypto_service.verify_password("wrong_password", &password_hash).await.unwrap();
    assert!(!is_invalid);
}
...
async fn test_password_hash_security() {
    ...
    assert!(!crypto.verify_password(different_password, &hash1).await.unwrap());
}
...
async fn test_complete_auth_flow_integration() {
    ...
    let login_valid = crypto_service.verify_password(password, &password_hash).await.unwrap();
    assert!(login_valid);
    
    // 8. Test invalid login
    let login_invalid = crypto_service.verify_password("wrong_password", &password_hash).await.unwrap();
