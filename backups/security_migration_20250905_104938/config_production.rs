...
impl Default for SecurityConfig {
    fn default() -> Self {
        // For production use, secrets MUST be provided via environment variables
        let jwt_secret = std::env::var("JWT_SECRET")
            .unwrap_or_else(|_| "INSECURE_DEV_SECRET_REPLACE_IN_PROD".to_string());
        let request_signing_secret = std::env::var("REQUEST_SIGNING_SECRET")
            .unwrap_or_else(|_| "INSECURE_DEV_SECRET_REPLACE_IN_PROD".to_string());
        let token_binding_salt = std::env::var("TOKEN_BINDING_SALT")
            .unwrap_or_else(|_| "INSECURE_DEV_SALT_REPLACE_IN_PROD".to_string());
        let encryption_key = std::env::var("ENCRYPTION_KEY")
            .unwrap_or_else(|_| "INSECURE_DEV_KEY_REPLACE_IN_PROD".to_string());
...
impl ConfigLoader {
    ...
    fn validate_config(config: &ProductionConfig) -> Result<()> {
        ...
        if config.security.jwt_secret.contains("INSECURE_DEV_SECRET") {
            return Err(anyhow::anyhow!(
