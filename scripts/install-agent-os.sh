...
install_agent_os() {
    ...
    if [[ -f "config/agent-os-integration.yaml" ]]; then
        print_info "Applying Agent OS integration configuration..."
        kubectl apply -f config/agent-os-integration.yaml -n "$NAMESPACE"
        print_success "Agent OS integration configuration applied"
    else
        print_error "config/agent-os-integration.yaml not found"
        exit 1
    fi
    
    # Apply monitoring configuration
    if [[ -f "config/agent-os-monitoring.yaml" ]]; then
        print_info "Applying Agent OS monitoring configuration..."
        kubectl apply -f config/agent-os-monitoring.yaml -n "$NAMESPACE"
        print_success "Agent OS monitoring configuration applied"
    else
        print_warning "config/agent-os-monitoring.yaml not found - skipping monitoring setup"
    fi
    
    # Apply service discovery configuration
    if [[ -f "config/agent-os-service-discovery.yaml" ]]; then
        print_info "Applying Agent OS service discovery configuration..."
        kubectl apply -f config/agent-os-service-discovery.yaml -n "$NAMESPACE"
        print_success "Agent OS service discovery configuration applied"
    else
        print_warning "config/agent-os-service-discovery.yaml not found - skipping service discovery setup"
    fi
...
show_next_steps() {
    ...
    echo "  - config/agent-os-integration.yaml (main configuration)"
    echo "  - config/agent-os-monitoring.yaml (monitoring setup)"
    echo "  - config/agent-os-service-discovery.yaml (service discovery)"
    echo
...
uninstall_agent_os() {
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        # Remove resources
        for file in config/agent-os-integration.yaml config/agent-os-monitoring.yaml config/agent-os-service-discovery.yaml; do
            if [[ -f "$file" ]]; then
