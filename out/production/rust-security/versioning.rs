...
use crate::{errors::VersioningError, ApiConfig};
...
impl ApiVersion {
    ...
    pub fn deprecation_date(
        &self,
        deprecated_versions: &HashMap<ApiVersion, DateTime<Utc>>,
    ) -> Option<DateTime<Utc>> {
...
impl VersionManager {
    ...
    pub fn deprecate_version(&mut self, version: ApiVersion) -> Result<(), VersioningError> {
        ...
        self.deprecated_versions
            .insert(version.clone(), deprecation_date);
    ...
    pub fn sunset_version(&mut self, version: &ApiVersion) -> Result<(), VersioningError> {
        if let Some(deprecation_date) = self.deprecated_versions.get(version) {
            let sunset_date = *deprecation_date
                + chrono::Duration::days(self.deprecation_policy.sunset_period_days as i64);
            ...
            if Utc::now() >= sunset_date {
                ...
                Err(VersioningError::TooEarlyForSunset(
                    version.clone(),
                    sunset_date,
                ))
            }
...
pub fn init_versioning(config: &ApiConfig) -> Result<VersionManager, VersioningError> {
    let mut manager =
        VersionManager::new(config.current_version.clone(), DeprecationPolicy::default());
...
mod tests {
    ...
    fn test_versioned_endpoint() {
        let mut endpoint =
            VersionedEndpoint::new("/api/auth".to_string(), ApiVersion::new(1, 0, 0));
