...
async fn voice_phone_social_engineering(
    ...
    scenario_data.insert(
        "attack_vectors_tested".to_string(),
        json!([
            "Phone enumeration",
...
async fn physical_social_engineering(
    ...
    scenario_data.insert(
        "security_controls_tested".to_string(),
        json!([
...
async fn digital_pretexting_advanced(
    ...
    for tech in &config.known_technologies {
        ...
        for scenario in &pretext_scenarios {
            ...
            let support_tests =
                test_support_pretexting(framework, scenario, config, &session).await?;
    ...
    for (i, subject) in subjects.iter().enumerate() {
        for email in &config.target_emails {
            templates.push(PhishingTemplate {
                ...
                pretext: format!(
                    "Urgent security matter requiring immediate attention for {}",
                    email
                ),
            });
        }
...
async fn test_bec_scenarios(
    ...
    for exec_email in &executive_emails {
        ...
        let fake_login_url = format!(
            "https://secure-{}-login.com/oauth/authorize",
...
async fn test_voice_authentication_bypass(
    ...
    for endpoint in &voice_endpoints {
        ...
        for payload in &voice_payloads {
            ...
            if result.response_body.contains("verified")
                && !result.response_body.contains("biometric")
            {
                results.push(format!(
                    "Weak voice verification at: {} - no biometric validation",
                    endpoint
                ));
            }
        }
...
async fn test_badge_authentication(
    ...
    for (badge_id, badge_type) in &badge_tests {
        ...
        if result.response_body.contains("valid") && !result.response_body.contains("biometric") {
            results
                .push(format!("Weak badge validation: {} - no additional verification", badge_id));
        }
    }
...
async fn test_api_pretexting(
    ...
    if result.success && !result.response_body.contains("unauthorized") {
        successful_attacks += 1;
        results.push(format!(
            "API pretexting successful: {} - {}",
            scenario.context, scenario.target_endpoint
        ));
    }
...
async fn test_administrative_pretexting(
    ...
    for (pretext, endpoint) in &admin_scenarios {
        ...
        if result.success && result.response_body.contains("access_granted") {
            successful_attacks += 1;
            results
                .push(format!("Administrative pretexting successful: {} at {}", pretext, endpoint));
        }
    }
...
async fn test_vendor_impersonation(
    ...
    for (vendor_type, service_claim) in &vendor_types {
        ...
        if result.success && result.response_body.contains("authorized") {
            successful_attacks += 1;
            results.push(format!(
                "Vendor impersonation successful: {} - {}",
                vendor_type, service_claim
            ));
        }
    }
...
async fn test_emergency_pretexting(
    ...
    for (emergency_type, action_required) in &emergency_scenarios {
        ...
        if result.success && result.response_body.contains("emergency_access") {
            successful_attacks += 1;
            results.push(format!(
                "Emergency pretexting successful: {} - {}",
                emergency_type, action_required
            ));
        }
    }
        ...
        for endpoint in &endpoints {
            ...
            let result = framework
                .execute_attack(
                    ...
                    "GET",
                    &format!(
                        "/recon/domain-probe?target={}&endpoint={}",
            ...
            if result.success && result.response_body.len() > 100 {
                successful_recon += 1;
                results
                    .push(format!("Active subdomain discovered: {} at {}", test_domain, endpoint));

                // Check for sensitive information disclosure
                let response_lower = result.response_body.to_lowercase();
                if response_lower.contains("admin")
                    || response_lower.contains("password")
                    || response_lower.contains("token")
                    || response_lower.contains("key")
                {
                    results.push(format!(
                        "Sensitive information disclosed at: {}{}",
                        test_domain, endpoint
                    ));
                }
            }
        }
...
async fn test_email_enumeration(
    ...
    for email in &config.target_emails {
        ...
        for endpoint in &validation_endpoints {
            ...
            let response_lower = result.response_body.to_lowercase();
            if response_lower.contains("exists")
                || response_lower.contains("found")
                || response_lower.contains("valid")
                || response_lower.contains("registered")
            {
                successful_recon += 1;
            ...
            {
                results.push(format!(
                    "Email enumeration via error messages: {} at {}",
                    email, endpoint
                ));
            }
        }
...
async fn test_employee_enumeration(
    ...
    for name in &common_names {
        ...
        if result.success && result.response_body.contains("profile") {
            ...
            let response_lower = result.response_body.to_lowercase();
            if response_lower.contains("department")
                || response_lower.contains("phone")
                || response_lower.contains("title")
...
async fn test_technology_disclosure(
    ...
    for endpoint in &tech_endpoints {
        ...
        if result.success && result.response_body.len() > 50 {
            ...
            if response_lower.contains("version")
                || response_lower.contains("build")
            ...
            let frameworks =
                vec!["react", "angular", "vue", "django", "rails", "express", "spring"];
            for framework_name in &frameworks {
                if response_lower.contains(framework_name) {
                    results
                        .push(format!("Framework disclosure: {} at {}", framework_name, endpoint));
                }
            }
        }
    }
...
async fn test_api_documentation_disclosure(
    ...
    for endpoint in &doc_endpoints {
        ...
        if result.success && result.response_body.len() > 500 {
            ...
            if response_lower.contains("authentication")
                || response_lower.contains("authorization")
                || response_lower.contains("token")
                || response_lower.contains("oauth")
            {
...
async fn test_certificate_intelligence(
    ...
    if result.success {
        ...
        if result.response_body.contains("infrastructure")
            || result.response_body.contains("ip_addresses")
        {
    ...
    for breach_db in &config.breach_databases {
        for email in &config.target_emails {
            ...
            if result.success && result.response_body.contains("found") {
                ...
                if result.response_body.contains("password")
                    || result.response_body.contains("credentials")
                {
...
async fn test_metadata_extraction(
    ...
    for (resource_type, endpoint) in &resource_types {
        ...
        if result.success && result.response_body.contains("metadata") {
            ...
            let response_lower = result.response_body.to_lowercase();
            if response_lower.contains("username")
                || response_lower.contains("author")
                || response_lower.contains("computer")
                || response_lower.contains("path")
            {
...
async fn test_business_intelligence(
    ...
    for endpoint in &business_endpoints {
        ...
        if result.success && result.response_body.len() > 200 {
            ...
            if response_lower.contains("department")
                || response_lower.contains("division")
                || response_lower.contains("team")
                || response_lower.contains("manager")
            {
