...
async fn phishing_simulation(
    ...
    let phishing_attempts = vec![
        ("credential_harvesting", "/oauth/token", "grant_type=client_credentials&client_id=user@company.com&client_secret=password123"),
        ("fake_reset", "/oauth/token", "grant_type=password&username=admin&password=reset123"),
        ("social_login", "/oauth/authorize", "response_type=code&client_id=facebook&redirect_uri=http://evil.com"),
    ];
    ...
    for (attack_type, endpoint, payload) in phishing_attempts {
        ...
        if response_lower.contains("user") || 
           ...
           response_lower.contains("invalid") && response_lower.contains("credentials") {
            phishing_results.push(format!("Information disclosure in {} attack: potential user enumeration", attack_type));
        }
...
async fn pretexting_attacks(
    ...
    for (scenario_name, headers) in pretexting_scenarios {
        ...
        for (header_name, header_value) in headers {
            request_headers.insert(header_name, reqwest::header::HeaderValue::from_str(header_value)?);
        }
        ...
        let token_result = framework.execute_attack(
            "pretexting_token_attack",
            "POST",
            "/oauth/token",
            Some(token_headers),
            Some("grant_type=client_credentials&client_id=internal&client_secret=bypass".to_string()),
            Some(&session),
        ).await?;
        
        if token_result.success {
            pretexting_results.push(format!("Token endpoint pretexting successful: {}", scenario_name));
        }
    }
    ...
    reporter.add_scenario_result("pretexting_attacks", pretexting_results.is_empty(), scenario_data);
    Ok(())
}
...
async fn information_disclosure_tests(
    ...
    for (endpoint, description) in test_endpoints {
        ...
        if result.success && result.response_body.len() > 50 {
            ...
            if response_lower.contains("key") && (response_lower.contains("private") || response_lower.contains("secret")) {
            ...
            if response_lower.contains("user") && (response_lower.contains("admin") || response_lower.contains("email")) {
            ...
            if !sensitive_info.is_empty() {
                disclosure_results.push(format!("{} ({}): {}", description, endpoint, sensitive_info.join(", ")));
                ...
                disclosure_results.push(format!("{} ({}): verbose response ({} chars)", description, endpoint, result.response_body.len()));
            }
        }
    }
    ...
    reporter.add_scenario_result("information_disclosure_tests", disclosure_results.is_empty(), scenario_data);
    Ok(())
}
...
async fn user_enumeration_attacks(
    ...
    let mut test_usernames = vec![
        ...
        "service", "system", "api", "oauth", "auth", "support", "help",
        "info", "contact", "sales", "marketing", "hr", "it",
    ];
    ...
    for (endpoint, test_type) in enumeration_endpoints {
        ...
        for username in &test_usernames[..user_count.min(test_usernames.len())] {
            ...
            let (body, description) = match test_type {
                "client_credentials" => (
                    format!("grant_type=client_credentials&client_id={}&client_secret=test", username),
            ...
            timing_differences.push((username, response_time, result.http_status, result.response_body.len()));
            ...
            if response_lower.contains("user not found") ||
               ...
               (response_lower.contains("invalid") && response_lower.contains("credentials") && !response_lower.contains("client")) {
                enumeration_results.push(format!("User enumeration via error message in {} for user: {}", description, username));
            }
        ...
        if timing_differences.len() >= 5 {
            let avg_time: u128 = timing_differences.iter().map(|(_, time, _, _)| time.as_millis()).sum::<u128>() / timing_differences.len() as u128;
            let significant_differences: Vec<_> = timing_differences.iter()
                .filter(|(_, time, _, _)| {
        ...
        let response_sizes: Vec<_> = timing_differences.iter().map(|(_, _, _, size)| *size).collect();
        let unique_sizes: std::collections::HashSet<_> = response_sizes.iter().collect();
        if unique_sizes.len() > 1 {
            enumeration_results.push(format!("Response size variation in {} may allow user enumeration", test_type));
        }
    }
    ...
    reporter.add_scenario_result("user_enumeration_attacks", enumeration_results.is_empty(), scenario_data);
    Ok(())
}
