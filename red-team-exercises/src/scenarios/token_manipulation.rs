...
async fn jwt_manipulation_attacks(
    ...
    for (attack_type, malicious_jwt) in jwt_attacks {
        ...
        headers.insert("Authorization", reqwest::header::HeaderValue::from_str(&format!("Bearer {}", malicious_jwt))?);
    ...
    reporter.add_scenario_result("jwt_manipulation_attacks", manipulation_results.is_empty(), scenario_data);
    Ok(())
}
...
async fn token_substitution_attacks(
    ...
    for token in substitute_tokens {
        let mut headers = reqwest::header::HeaderMap::new();
        headers.insert("Authorization", reqwest::header::HeaderValue::from_str(&format!("Bearer {}", token))?);
    ...
    for auth_type in auth_types {
        let mut headers = reqwest::header::HeaderMap::new();
        headers.insert("Authorization", reqwest::header::HeaderValue::from_str(&format!("{} admin_token", auth_type))?);
        ...
        if result.success {
            substitution_results.push(format!("Authorization type substitution successful: {}", auth_type));
        }
    }
    ...
    reporter.add_scenario_result("token_substitution_attacks", substitution_results.is_empty(), scenario_data);
    Ok(())
}
...
async fn token_replay_attacks(
    ...
    if let Ok((valid_token, _)) = framework.attempt_client_credentials_flow("test", "test").await {
        ...
        let mut headers = reqwest::header::HeaderMap::new();
        headers.insert("Authorization", reqwest::header::HeaderValue::from_str(&format!("Bearer {}", valid_token))?);
    ...
    for expired_token in expired_tokens {
        let mut headers = reqwest::header::HeaderMap::new();
        headers.insert("Authorization", reqwest::header::HeaderValue::from_str(&format!("Bearer {}", expired_token))?);
...
async fn token_enumeration_attacks(
    ...
    for pattern in token_patterns {
        ...
        for i in 0..(range / token_patterns.len()) {
            ...
            headers.insert("Authorization", reqwest::header::HeaderValue::from_str(&format!("Basic {}", auth))?);
            ...
            if result.success {
                if let Ok(response_json) = serde_json::from_str::<serde_json::Value>(&result.response_body) {
        ...
        if !valid_tokens.is_empty() {
            enumeration_results.push(format!("Token enumeration successful with pattern '{}': {} valid tokens", pattern, valid_tokens.len()));
        }
    }
    ...
    for pattern in uuid_patterns {
        for i in 0..10 {
            ...
            if result.success {
                enumeration_results.push(format!("UUID-pattern token enumeration successful: {}", token));
            }
        }
    }
    ...
    scenario_data.insert("patterns_tested".to_string(), json!(token_patterns.len() + uuid_patterns.len()));
    
    reporter.add_scenario_result("token_enumeration_attacks", enumeration_results.is_empty(), scenario_data);
    Ok(())
}
