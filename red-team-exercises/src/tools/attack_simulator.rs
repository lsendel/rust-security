...
impl AttackSimulator {
    ...
    async fn simulate_credential_stuffing(&self, target: &str, config: &ToolConfig) -> Result<AttackMetrics> {
    ...
    async fn simulate_brute_force(&self, target: &str, config: &ToolConfig) -> Result<AttackMetrics> {
    ...
    async fn simulate_idor_testing(&self, target: &str, config: &ToolConfig) -> Result<AttackMetrics> {
    ...
    fn generate_credential_list(&self, intensity: &str) -> Vec<(String, String)> {
        ...
        match intensity {
            "high" => {
                let mut extended = base_credentials.into_iter().map(|(u, p)| (u.to_string(), p.to_string())).collect::<Vec<_>>();
                for i in 0..100 {
            ...
            "medium" => {
                let mut medium = base_credentials.into_iter().map(|(u, p)| (u.to_string(), p.to_string())).collect::<Vec<_>>();
                for i in 0..20 {
                    medium.push((format!("test{}", i), "password".to_string()));
                }
                medium
            }
            _ => base_credentials.into_iter().map(|(u, p)| (u.to_string(), p.to_string())).collect(),
        }
    }
    ...
    fn generate_password_list(&self, intensity: &str) -> Vec<String> {
        ...
        match intensity {
            "high" => {
                let mut extended = base_passwords.into_iter().map(|s| s.to_string()).collect::<Vec<_>>();
                extended.extend(vec![
            ...
            "medium" => {
                let mut medium = base_passwords.into_iter().map(|s| s.to_string()).collect::<Vec<_>>();
                medium.extend(vec!["Password1", "admin123", "welcome123"].into_iter().map(|s| s.to_string()));
                medium
            }
        ...
        match intensity {
            "high" => {
                for i in 0..1000 {
                    ...
                    session_ids.push(format!("{:08x}-{:04x}-{:04x}-{:04x}-{:012x}", 
...
impl RedTeamTool for AttackSimulator {
    ...
    async fn execute(&self, target: &str, config: &ToolConfig) -> Result<ToolResult> {
        ...
        // Combine metrics
        let total_attempts = credential_stuffing.total_attempts() + brute_force.total_attempts() + idor_testing.total_attempts();
        let total_successful = credential_stuffing.successful_attempts() + brute_force.successful_attempts() + idor_testing.successful_attempts();
        let total_detected = credential_stuffing.detected_attempts() + brute_force.detected_attempts() + idor_testing.detected_attempts();
        
        let success_rate = if total_attempts > 0 { total_successful as f64 / total_attempts as f64 } else { 0.0 };
        let detection_rate = if total_attempts > 0 { total_detected as f64 / total_attempts as f64 } else { 0.0 };
        ...
        if success_rate > 0.1 {
            findings.push("High attack success rate detected - security controls may be insufficient".to_string());
        }
        
        if detection_rate < 0.5 {
            findings.push("Low detection rate - monitoring and alerting may need improvement".to_string());
        }
        
        if credential_stuffing.successful_attempts() > 0 {
            findings.push(format!("Credential stuffing successful: {} attempts", credential_stuffing.successful_attempts()));
        }
        
        if brute_force.successful_attempts() > 0 {
            findings.push(format!("Brute force successful: {} attempts", brute_force.successful_attempts()));
        }
        
        if idor_testing.successful_attempts() > 0 {
            findings.push(format!("IDOR vulnerabilities found: {} accessible resources", idor_testing.successful_attempts()));
        }
