...
impl PerformanceMonitor {
    ...
    async fn measure_under_attack(&self, target: &str, config: &ToolConfig) -> Result<PerformanceMetrics> {
    ...
    fn analyze_performance_impact(
        ...
        let error_rate_under_attack = if total_requests > 0 {
        ...
        let legit_response_times: Vec<u64> = under_attack.legitimate_traffic
            .iter()
        ...
        let legit_error_count = under_attack.legitimate_traffic
            .iter()
            .filter(|(_, success)| !*success)
            .count();
        ...
        let response_time_degradation = if baseline.avg_response_time > 0 {
            (avg_legit_response_time as f64 - baseline.avg_response_time as f64) / baseline.avg_response_time as f64
        } else {
            0.0
        };
...
impl RedTeamTool for PerformanceMonitor {
    ...
    async fn execute(&self, target: &str, config: &ToolConfig) -> Result<ToolResult> {
        ...
        metrics.insert("baseline_response_time_ms".to_string(), analysis.baseline_avg_response_time as f64);
        metrics.insert("attack_response_time_ms".to_string(), analysis.attack_avg_response_time as f64);
        metrics.insert("legit_response_time_ms".to_string(), analysis.legit_avg_response_time as f64);
        ...
        metrics.insert("service_available".to_string(), if analysis.service_available { 1.0 } else { 0.0 });
        
        // Generate findings
        let mut findings = Vec::new();
        
        if analysis.response_time_degradation > 0.5 {
            findings.push(format!("Significant performance degradation: {:.1}% increase in response time", analysis.response_time_degradation * 100.0));
        }
        
        if analysis.error_rate_increase > 0.2 {
            findings.push(format!("High error rate increase: {:.1}% during attack", analysis.error_rate_increase * 100.0));
        }
        ...
        if analysis.requests_per_second > 100.0 {
            findings.push(format!("High attack rate achieved: {:.1} requests/second", analysis.requests_per_second));
        }
