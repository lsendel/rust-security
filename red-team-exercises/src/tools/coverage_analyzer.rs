...
impl CoverageAnalyzer {
    ...
    async fn analyze_endpoint_security(&self, target: &str, endpoint: &str) -> Result<EndpointSecurityAnalysis> {
        ...
        if let Ok(response) = unauth_result {
            ...
            if status == 200 && response.content_length().unwrap_or(0) > 0 {
                analysis.security_issues.push("Endpoint accessible without authentication".to_string());
            }
        }
        
        // Test 2: Test with invalid authentication
        let invalid_auth_result = self.client
            .get(&format!("{}{}", target, endpoint))
        ...
        if let Ok(response) = invalid_auth_result {
            ...
            if status == 200 {
                analysis.security_issues.push("Endpoint accepts invalid authentication".to_string());
            }
        }
        
        // Test 3: Check for verbose error messages
        let malformed_result = self.client
        ...
        if let Ok(response) = malformed_result {
            ...
            if body.len() > 200 && (
                ...
                analysis.security_issues.push("Verbose error messages may leak information".to_string());
            }
        }
        ...
        for method in methods {
            let result = match method {
                ...
                "OPTIONS" => self.client.request(reqwest::Method::OPTIONS, &format!("{}{}", target, endpoint)).send().await,
                _ => continue,
            };
        ...
        let headers_result = self.client
            .get(&format!("{}{}", target, endpoint))
            .send()
            .await;
    ...
    fn analyze_attack_surface(&self, endpoint_analyses: &[EndpointSecurityAnalysis]) -> AttackSurfaceAnalysis {
        let total_endpoints = endpoint_analyses.len();
        let unauthenticated_endpoints = endpoint_analyses.iter()
            .filter(|a| !a.requires_authentication)
            .count();
        
        let high_risk_endpoints = endpoint_analyses.iter()
            .filter(|a| a.security_score < 0.5)
            .count();
        
        let endpoints_with_issues = endpoint_analyses.iter()
            .filter(|a| !a.security_issues.is_empty())
            .count();
        
        let total_security_issues: usize = endpoint_analyses.iter()
            .map(|a| a.security_issues.len())
            .sum();
        
        let admin_endpoints = endpoint_analyses.iter()
            .filter(|a| a.endpoint.contains("/admin/"))
            .count();
        
        let mfa_endpoints = endpoint_analyses.iter()
            .filter(|a| a.endpoint.contains("/mfa/"))
            .count();
        
        let oauth_endpoints = endpoint_analyses.iter()
            .filter(|a| a.endpoint.contains("/oauth/"))
            .count();
...
impl RedTeamTool for CoverageAnalyzer {
    ...
    async fn execute(&self, target: &str, config: &ToolConfig) -> Result<ToolResult> {
        ...
        metrics.insert("unauthenticated_endpoints".to_string(), attack_surface.unauthenticated_endpoints as f64);
        metrics.insert("high_risk_endpoints".to_string(), attack_surface.high_risk_endpoints as f64);
        metrics.insert("total_security_issues".to_string(), attack_surface.total_security_issues as f64);
        metrics.insert("admin_endpoints".to_string(), attack_surface.admin_endpoints as f64);
        metrics.insert("coverage_percentage".to_string(), 
            if attack_surface.total_endpoints > 0 { 
                (attack_surface.total_endpoints - attack_surface.high_risk_endpoints) as f64 / attack_surface.total_endpoints as f64 * 100.0 
            } else { 
                0.0 
            }
        );
        ...
        if attack_surface.unauthenticated_endpoints > 0 {
            findings.push(format!("{} endpoints accessible without authentication", attack_surface.unauthenticated_endpoints));
        }
        
        if attack_surface.high_risk_endpoints > 0 {
            findings.push(format!("{} high-risk endpoints identified", attack_surface.high_risk_endpoints));
        }
        
        if attack_surface.total_security_issues > 10 {
            findings.push(format!("High number of security issues found: {}", attack_surface.total_security_issues));
        }
        
        // Add specific endpoint issues
        for analysis in &endpoint_analyses {
            if !analysis.security_issues.is_empty() {
                findings.push(format!("{}: {}", analysis.endpoint, analysis.security_issues.join(", ")));
            }
        }
        
        // Success if security posture is good and no critical issues
        let success = attack_surface.security_posture == "Excellent" || 
                     (attack_surface.security_posture == "Good" && attack_surface.high_risk_endpoints == 0);
