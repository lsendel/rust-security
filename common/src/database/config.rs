...
impl Default for PostgresConfig {
    fn default() -> Self {
        Self {
            url: "postgresql://localhost/rust_security".to_string(),
            max_connections: 10,
            connect_timeout: Duration::from_secs(5),
            query_timeout: Duration::from_secs(30),
            ssl_mode: "prefer".to_string(),
            database_name: "rust_security".to_string(),
    ...
    pub fn from_env() -> DatabaseResult<Self> {
        ...
        if let Ok(postgres_url) = env::var("DATABASE_URL") {
            ...
            if let Ok(max_conns) = env::var("DATABASE_MAX_CONNECTIONS") {
                pg_config.max_connections = max_conns.parse()
                    .map_err(|_| DatabaseError::ConfigurationError("Invalid DATABASE_MAX_CONNECTIONS".to_string()))?;
            }
        ...
        if env::var("REDIS_URL").is_ok() {
            config.database.redis = Some(UnifiedRedisConfig::from_env()
                .map_err(|e| DatabaseError::ConfigurationError(format!("Redis config error: {}", e)))?);
        }
        ...
        if !validation_result.valid {
            return Err(DatabaseError::ValidationError(validation_result.errors.join(", ")));
        }
