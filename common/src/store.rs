...
pub trait Store: Send + Sync {
    ...
    async fn create_group(
        &self,
        group: &ScimGroup,
    ) -> Result<ScimGroup, Box<dyn Error + Send + Sync>>;
    ...
    async fn consume_auth_code(
        &self,
        code: &str,
    ) -> Result<Option<AuthCodeRecord>, Box<dyn Error + Send + Sync>>;

    // === Token Management ===
    async fn get_token_record(
        &self,
        token: &str,
    ) -> Result<Option<TokenRecord>, Box<dyn Error + Send + Sync>>;
    ...
    async fn get_metrics(&self)
        -> Result<crate::types::StoreMetrics, Box<dyn Error + Send + Sync>>;
}
